...
First, we need to parse the list to determine which taxa in the matrix (‘m’) is a descendant of Ictalurus. This returns a list of TRUE or FALSE assignments for each taxon in the matrix:
...
```{r}
to_keep <- c(TRUE, TRUE, TRUE, is_desc) 
pecfin_m <- m[, to_keep]
```

...
td.comb <- recode_traits(tdf, amal.deps) ## Recode the original dataset according to the character states of the new, amalgamated characters
```
...
We are going to reconstruct our stochastic character maps by first fitting the model in the R package `corHMM`, then using the model fit to generate stochastic maps in `phytools`. In the workshop, we will do a Julia Child-style switch-a-roo and analyze completed analyses, rather than actually running these, because they will take too long. 
...
```{r}
data(corhmm.fits)
trees <- amalgamated_simmaps_phytools(corhmm.fits, nsim=2)
```

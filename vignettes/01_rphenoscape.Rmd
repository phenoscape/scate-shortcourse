...
First let’s look up information about the query terms. You can use the term detail functions ```anatomy_term_info()```, ```taxon_info()```, ```phenotypic_quality_term_info()```, and ```gene_info()``` to retrieve an ontology term’s ID, label, and definition. 

For example, view the details for the anatomy term “fin”:

```{r}
anatomy_term_info("fin")
```
...
```{r}
taxon_info("Siluriformes")
```

Ontology terms are related to each other by superclass (parent) and subclass (child) relationships, among other relationships such as part_of. You can use ```term_classification()```to view the superclass and subclass relationships for a given term (e.g., anatomy, taxon).

For example, view the relationships for “pectoral fin”: 

```{r}
term_classification("pectoral fin", as = "anatomy")
```
...
```
The above returns 29 species of ictalurid catfishes and 127 anatomical entities for fins and their parts.  
...
[faceted browsing page]: https://kb.phenoscape.org/facet?quality=&publicationsPage=1&tab=taxa&geneAnnotationsPage=1&includeParts=false&includeSerialHomologs=false&includeHistoricalHomologs=false&phenotypesPage=1&taxon=&genesPage=1&taxaAnnotationsPage=1&taxaPage=1&entity=&qualityMode=quality
